...
exports.executeCPPDemo = () => {
    return new Promise((resolve, reject) => {
        ...
        const runCmd = `"${outputBinary}"`;

        // Check if file exists
        if (!fs.existsSync(cppFilePath)) {
            return reject(new Error('C++ Source file not found at ' + cppFilePath));
        }
        ...
        exec(compileCmd, (compileErr) => {
            ...
            exec(runCmd, (runErr, stdout, stderr) => {
                if (runErr) {
                    ...
                    const linuxRunCmd = `g++ "${cppFilePath}" -o logic_demo && ./logic_demo`;
                    exec(linuxRunCmd, (linuxErr, lStdout, lStderr) => {
                        if (linuxErr) {
                            return resolve("C++ Logic Engine: [OFFLINE] (Install g++ to enable real-time C++ hardware acceleration)");
                        }
                        resolve(lStdout);
                    });
                } else {
